use bevy::{
    math::{DMat3, DMat4, DVec4},
    prelude::*,
};
use bevy_mod_scripting::{
    mlu::mlua::{Lua, UserData},
    AddScriptHost, LuaDMat3, LuaDMat4, LuaValue, LuaVec3, LuaWrapper, RLuaScriptHost, ScriptHost,
    ScriptingPlugin,
};
...
fn mat_transpose(c: &mut Criterion) {
    ...
    .iter()
    {
        ...
        group.bench_function("Ref", |b| b.iter(|| wrapped.val(|s| s.transpose())));

        group.bench_function("Copy", |b| b.iter(|| wrapped.inner().transpose()));

        group.bench_function("Baseline", |b| b.iter(|| i.transpose()));
    }
}
...
fn mat_inverse(c: &mut Criterion) {
    ...
    .iter()
    {
        ...
        group.bench_function("Ref", |b| b.iter(|| wrapped.val(|s| s.inverse())));

        group.bench_function("Copy", |b| b.iter(|| wrapped.inner().inverse()));

        group.bench_function("Baseline", |b| b.iter(|| i.inverse()));
    }
}
