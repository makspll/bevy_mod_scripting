...
use bevy::{prelude::*, reflect::Reflect, render::{render_resource::{Extent3d, TextureDimension, TextureFormat, Texture, SamplerDescriptor}, texture::ImageSampler}, window::{WindowResized, WindowCreated}, time::FixedTimestep, diagnostic::{FrameTimeDiagnosticsPlugin, LogDiagnosticsPlugin}};
use bevy_mod_scripting::{
    prelude::*,
    api::lua::bevy::LuaBevyAPIProvider,ReflectLuaProxyable,
    langs::mlu::mlua, std::LuaVec, lua_path
};
...
pub fn sync_window_size(
    ...
    if resize_event.iter().filter(|w| w.id.is_primary()).last().is_some() {
...
fn main() -> std::io::Result<()> {
    ...
    app.add_plugins(DefaultPlugins)
        ...
        .add_stage_after(CoreStage::Update, LifeStages::Scripts, SystemStage::single_threaded())
