...
use std::ops::*;
use crate::{FromLuaProxy, ReflectPathElem};
use crate::{RegisterForeignLuaType,LuaProxyable,ScriptRef,ReflectBase,ReflectedValue, api::ValueIndex, APIProvider, LuaDocFragment};
...
mut (MetaMethod::Index) => |_,s,idx : usize| {
	Ok(LuaVec3::new_ref(
			s.script_ref().sub_ref(ReflectPathElem::IndexedSubReflection{
				label:"col", 
				index: idx,
				get: |idx,ref_| panic!("Cannot get mutable column reference given immutable matrix reference"),
				get_mut: |idx, ref_| ref_.downcast_mut::<Mat3>().unwrap().col_mut(idx)
			})
		)
	)
}
...
mut (MetaMethod::Index) => |_,s,idx : usize| {
	Ok(LuaVec2::new_ref(
			s.script_ref().sub_ref(ReflectPathElem::IndexedSubReflection{
				...
				index: idx,
				get: |idx,ref_| panic!("Cannot get mutable column reference given immutable matrix reference"),
				get_mut: |idx, ref_| ref_.downcast_mut::<Mat2>().unwrap().col_mut(idx)
			})
		)
	)
}
...
mut (MetaMethod::Index) => |_,s,idx : usize| {
	Ok(LuaVec3A::new_ref(
			s.script_ref().sub_ref(ReflectPathElem::IndexedSubReflection{
				...
				index: idx,
				get: |idx,ref_| panic!("Cannot get mutable column reference given immutable matrix reference"),
				get_mut: |idx, ref_| ref_.downcast_mut::<Mat3A>().unwrap().col_mut(idx)
			})
		)
	)
}
...
mut (MetaMethod::Index) => |_,s,idx : usize| {
	Ok(LuaVec4::new_ref(
			s.script_ref().sub_ref(ReflectPathElem::IndexedSubReflection{
				...
				index: idx,
				get: |idx,ref_| panic!("Cannot get mutable column reference given immutable matrix reference"),
				get_mut: |idx, ref_| ref_.downcast_mut::<Mat4>().unwrap().col_mut(idx)
			})
		)
	)
}
...
mut (MetaMethod::Index) => |_,s,idx : usize| {
	Ok(LuaDVec2::new_ref(
			s.script_ref().sub_ref(ReflectPathElem::IndexedSubReflection{
				...
				index: idx,
				get: |idx,ref_| panic!("Cannot get mutable column reference given immutable matrix reference"),
				get_mut: |idx, ref_| ref_.downcast_mut::<DMat2>().unwrap().col_mut(idx)
			})
		)
	)
}
...
mut (MetaMethod::Index) => |_,s,idx : usize| {
	Ok(LuaDVec3::new_ref(
			s.script_ref().sub_ref(ReflectPathElem::IndexedSubReflection{
				...
				index: idx,
				get: |idx,ref_| panic!("Cannot get mutable column reference given immutable matrix reference"),
				get_mut: |idx, ref_| ref_.downcast_mut::<DMat3>().unwrap().col_mut(idx)
			})
		)
	)
}
...
mut (MetaMethod::Index) => |_,s,idx : usize| {
	Ok(LuaDVec4::new_ref(
			s.script_ref().sub_ref(ReflectPathElem::IndexedSubReflection{
				...
				index: idx,
				get: |idx,ref_| panic!("Cannot get mutable column reference given immutable matrix reference"),
				get_mut: |idx, ref_| ref_.downcast_mut::<DMat4>().unwrap().col_mut(idx)
			})
		)
	)
}
