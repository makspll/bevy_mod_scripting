...
extern crate self as bevy_mod_scripting;
use bevy_mod_scripting_derive::impl_script_newtype;
use std::ops::*;
use crate::{lua::bevy::{LuaWorld,LuaTypeRegistration,LuaScriptData}, std::LuaVec, DummyTypeName};
use crate::{impl_tealr_any_union,ReflectionError,ReflectPathElem,RegisterForeignLuaType,ReflectedValue, api::ValueIndex, APIProvider, LuaDocFragment};
use std::sync::Mutex;
use crate::{impl_tealr_generic};
use tealr::{mlu::{mlua,mlua::{prelude::*,MetaMethod}}};
...
impl_tealr_generic!(pub(crate) struct T);
impl_tealr_generic!(pub(crate) struct K);
impl_tealr_generic!(pub(crate) struct V);
#[derive(Default)]
...
impl APIProvider for LuaBevyAPIProvider{
	...
	fn get_doc_fragment(&self) -> Option<Self::DocTarget>	{
		Some(LuaDocFragment::new(|tw|		{
			...
			.process_type::<tealr::mlu::UserDataProxy<LuaWorld>>()
			.process_type::<LuaScriptData>()
			.process_type::<tealr::mlu::UserDataProxy<LuaScriptData>>()
			.process_type::<LuaTypeRegistration>()
			.process_type::<LuaVec<T>>()
		}
		))
	}
