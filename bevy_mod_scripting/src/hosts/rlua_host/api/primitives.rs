...
impl UserData for LuaNumber {
    fn add_methods<'lua, T: rlua::UserDataMethods<'lua, Self>>(methods: &mut T) {
        methods.add_meta_method(rlua::MetaMethod::Add, |_, v, o: Value| {
            Ok(v.add(LuaNumber::from_lua(o, v.type_name()).unwrap())
                .unwrap())
        });
        methods.add_meta_method(rlua::MetaMethod::Sub, |_, v, o: Value| {
            Ok(v.sub(LuaNumber::from_lua(o, v.type_name()).unwrap())
                .unwrap())
        });
        methods.add_meta_method(rlua::MetaMethod::Mul, |_, v, o: Value| {
            Ok(v.mul(LuaNumber::from_lua(o, v.type_name()).unwrap())
                .unwrap())
        });
        methods.add_meta_method(rlua::MetaMethod::Div, |_, v, o: Value| {
            Ok(v.div(LuaNumber::from_lua(o, v.type_name()).unwrap())
                .unwrap())
        });
        methods.add_meta_method(rlua::MetaMethod::Mod, |_, v, o: Value| {
            Ok(v.rem(LuaNumber::from_lua(o, v.type_name()).unwrap())
                .unwrap())
        });
        methods.add_meta_method(rlua::MetaMethod::Pow, |_, v, o: Value| {
            Ok(v.pow(LuaNumber::from_lua(o, v.type_name()).unwrap())
                .unwrap())
        });
        ...
        methods.add_method_mut("as", |_, v, typ: String| Ok(v.as_type(&typ).unwrap()))
    }
}
...
pub mod tests {
    ...
    fn test_normal_ops() {
        assert_eq_lua(
            LuaNumber::Usize(0).add(LuaNumber::Usize(1)).unwrap(),
            LuaNumber::Usize(1),
        );
        assert_eq_lua(
            LuaNumber::U32(2).sub(LuaNumber::U32(1)).unwrap(),
            LuaNumber::U32(1),
        );
        assert_eq_lua(
            LuaNumber::U8(2).mul(LuaNumber::U8(1)).unwrap(),
            LuaNumber::U8(2),
        );
        assert_eq_lua(
            LuaNumber::I8(4).div(LuaNumber::I8(2)).unwrap(),
            LuaNumber::I8(2),
        );
        assert_eq_lua(
            LuaNumber::I64(3).rem(LuaNumber::I64(2)).unwrap(),
            LuaNumber::I64(1),
        );
        assert_eq_lua(
            LuaNumber::U16(3).pow(LuaNumber::U16(2)).unwrap(),
            LuaNumber::U16(9),
        );
    ...
    fn test_valid_casts() {
        assert_eq_lua(
            LuaNumber::Usize(0).as_type("isize").unwrap(),
            LuaNumber::Isize(0),
        );
        assert_eq_lua(
            LuaNumber::Isize(0).as_type("usize").unwrap(),
            LuaNumber::Usize(0),
        );

        assert_eq_lua(
            LuaNumber::U32(255).as_type("u8").unwrap(),
            LuaNumber::U8(255),
        );
        assert_eq_lua(
            LuaNumber::U8(255).as_type("u32").unwrap(),
            LuaNumber::U32(255),
        );

        assert_eq_lua(
            LuaNumber::F32(64.0).as_type("u8").unwrap(),
            LuaNumber::U8(64),
        );
        assert_eq_lua(
            LuaNumber::U8(64).as_type("f32").unwrap(),
            LuaNumber::F32(64.0),
        );

        assert_eq_lua(
            LuaNumber::F32(64.0).as_type("f64").unwrap(),
            LuaNumber::F64(64.0),
        );
        assert_eq_lua(
            LuaNumber::F64(64.0).as_type("f32").unwrap(),
            LuaNumber::F32(64.0),
        );
    }
