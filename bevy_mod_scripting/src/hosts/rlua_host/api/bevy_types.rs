...
// impl rlua::UserData for LuaVec2 {
//     fn add_methods<'lua, T: rlua::UserDataMethods<'lua, Self>>(methods: &mut T) {

//         methods.add_meta_function(rlua::MetaMethod::Sub, |_, (lhs, rhs): (Value, Value)| {
...
//             };
//             let ud: &LuaVec2 = &ud.borrow::<LuaVec2>().unwrap();
//             let op = if op_on_rhs { &rhs } else { &lhs };
//             if op_on_rhs {
//             } else {
//                 if let Value::UserData(op) = op {
//                     if let Ok(op) = op.borrow::<LuaVec2>() {
//                         let op = &op;
//                         return ud.bin(op, |ud, op| op.val(|op| Ok(ud.sub(*op))));
//                     }
//                 };
//             };
//             Err(rlua::Error::RuntimeError("asd".to_owned()))
//         });
//     }
// }
...
impl_lua_newtypes!{
    ...
    [ // wrappers
    {
        ...
        : 
            DebugToString
            + MathBinOp(Sub:LuaWrapper[(Rhs:LuaVec2)])


        impl {
            // fn (rlua::MetaMethod::Add) (a=u32) => {|_,arg : ($a)| Ok(())};
            // MetaMethod::Pow => |_,s : &[<Lua $vec_base>], o : $vec_float_inner| { Ok([<Lua $vec_base>]::new(s.val(|s| s.powf(o)))) };
            // MetaMethod::Unm => |_,s : &[<Lua $vec_base>],()| { Ok(([<Lua $vec_base>]::new(s.inner().neg()))) };
            // "abs" => |_,s,()| Ok(LuaVec2::new(s.inner().abs()));
            // "signum" => |_,s,()|  Ok(LuaVec2::new(s.inner().signum())); 
        }
    },
    {
        bevy_ecs::entity::Entity: Full 
            // :Copy(
            //     LuaVec2 -> fn (rlua::MetaMethod::Sub) (a=f64),
            // )
        impl {
