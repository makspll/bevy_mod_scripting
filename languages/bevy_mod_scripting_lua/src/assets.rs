use bevy::{
    asset::{AssetLoader, io::Reader, Asset, AsyncReadExt},
    reflect::{TypePath}, utils::{BoxedFuture},
    prelude::*
};
...
fn old_lua_load<'a>(bytes: &'a [u8],load_context: &'a mut bevy::asset::LoadContext)-> BoxedFuture<'a, Result<Vec<u8>, Error>>{
    match load_context.path().extension().map(|s| s.to_str().unwrap())  {
        ...
        Some("tl") => {
            ...
            info!("tl check {} : {}",full_path.to_str().unwrap(),scripts_dir.to_str().unwrap());
        ...
        _ => {
            Box::pin(async move {Ok(bytes.into())})
        }
    }
}
impl AssetLoader for LuaLoader {
    ...
    fn load<'a>(
        ...
        Box::pin(async move { 
            ...
            let bytes_vec = old_lua_load(bytes.as_slice(), load_context).await?;
            Ok(LuaFile{bytes: bytes_vec})//})
        })
    }
