...
pub(crate) fn generate_macros(
    ...
    for (global_name,type_, dummy_proxy) in wrapped_items
        .iter()
        .filter_map(|i| i.has_global_methods.then_some((i.wrapped_type.as_str(),i.wrapper_name.as_str(),false)))
        .chain(config.manual_lua_types
            .iter()
            .filter_map(|i| i.include_global_proxy.then_some((i.proxy_name.as_str(),i.name.as_str(),i.use_dummy_proxy)))
        ) 
    {
    ...
    writer.write_line("type APITarget = Mutex<Lua>;");
    writer.write_line("type ScriptContext = Mutex<Lua>;");
    ...
    for (type_,include_proxy) in wrapped_items
        .iter()
        .map(|i| (i.wrapper_name.as_str(),i.has_global_methods))
        .chain(config.manual_lua_types.iter()
            .filter_map(|i| (!i.dont_process).then_some((i.name.as_str(),i.include_global_proxy)))
        ) 
    {
