...
use bevy_api_gen_lib::{ArgType, Args, Config, Newtype, PrettyWriter, TypeMeta, WRAPPER_PREFIX};

use clap::Parser;
use cratepath::{
    get_path, lookup_external_item_crate_source_name, lookup_item_crate_source, path_to_import,
};
use indexmap::{IndexMap, IndexSet};
use rustdoc_types::{Crate, Impl, Item, ItemEnum, Type};
...
impl TryFrom<((String, Type), &Config)> for FunctionArg {
...
impl TryFrom<(Option<ImportPath>, Item, &Config)> for FunctionData {
    ...
    fn try_from(
        ...
        let is_static = args
            .first()
            .map(|first: &FunctionArg| !first.type_.is_receiver())
            .unwrap_or(true);
...
struct ItemData {
    ...
    functions: IndexMap<String, FunctionData>,
    docstrings: Vec<String>,
}
                            ...
                            .map(|trait_| {
                                ...
                                let origin_crate =
                                    lookup_item_crate_source(&trait_.id, meta.crates).ok_or(
                                        format!(
                                            "Could not find trait in any crates `{}`",
                                            trait_.name
                                        ),
                                    )?;
                                ...
                                let import_path =
                                    path_to_import(path_components, meta.source).into();
                                Ok(import_path)
                            })
                            .transpose()
...
struct TemplateData {
    items: IndexMap<String, ItemData>,
}
