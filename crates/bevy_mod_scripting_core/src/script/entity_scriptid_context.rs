...
impl<P: IntoScriptPluginParams> ScriptContextProvider<P> for EntityScriptIdContext<P> {
    fn hash(&self, context_key: &ContextKey) -> Option<u64> {
        context_key.entity.zip(context_key.script.as_ref()).map(|(id, script)| DefaultHashBuilder::default().hash_one((id, script.id())))
    }
    fn get(&self, context_key: &ContextKey) -> Option<&Arc<Mutex<P::C>>> {
        context_key.entity.zip(context_key.script.as_ref()).and_then(|(e, h)| self.0.get(&(e, h.id())))
    }
    ...
    fn contains(&self, context_key: &ContextKey) -> bool {
        context_key.entity.zip(context_key.script.as_ref()).map(|(e, h)| self.0.contains_key(&(e, h.id()))).unwrap_or(false)
    }
