...
impl ReflectReference {
    ...
    pub fn with_reflect<O, F: FnOnce(&dyn Reflect) -> O>(
        &self,
    ...
    pub fn with_reflect_mut<O, F: FnOnce(&mut dyn Reflect) -> O>(
        &self,
        world: &WorldAccessGuard,
        type_registry: &TypeRegistry,
        allocator: Option<&ReflectAllocator>,
    ...
    pub fn reflect_mut<'w, 'c>(
        ...
        type_registry: &TypeRegistry,
        allocator: Option<&'c ReflectAllocator>,
    ) -> ScriptResult<&'c mut dyn Reflect> {
    ...
    pub unsafe fn reflect_mut_unsafe<'w>(
        ...
        type_registry: &TypeRegistry,
        allocator: Option<&'w ReflectAllocator>,
    ) -> ScriptResult<&'w mut dyn Reflect> {
        if let ReflectBase::Owned(id) = &self.base.base_id {
        ...
        self.walk_path_mut(base)
    }

    fn walk_path<'a>(&self, root: &'a dyn Reflect) -> ScriptResult<&'a dyn Reflect> {
    ...
    fn walk_path_mut<'a>(&self, root: &'a mut dyn Reflect) -> ScriptResult<&'a mut dyn Reflect> {
        let mut current = root;
        for elem in self.reflect_path.iter() {
            current = elem
