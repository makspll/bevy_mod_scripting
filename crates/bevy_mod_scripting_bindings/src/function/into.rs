...
/// Converts a value into zero or many [`ScriptValue`]'s
pub trait IntoScriptMulti {
    /// Converts [`Self`] into an appropriate representation using a list of [`ScriptValue`]'s
    fn into_script_multi(self, world: WorldGuard) -> Result<VecDeque<ScriptValue>, InteropError>;
}
...
macro_rules! impl_into_script_multi_tuple {
    ($($ty:ident),*) => {
        #[allow(non_snake_case)]
        #[profiling::all_functions]
        impl<$($ty: IntoScript),*> IntoScriptMulti for ($($ty,)*)
        where
            Self: 'static,
        {
            fn into_script_multi(
                self,
                world: WorldGuard,
            ) -> Result<VecDeque<ScriptValue>, InteropError> {
                let ($($ty,)*) = self;

                let out = VecDeque::from_iter([
                    $(
                        $ty.into_script(world.clone())?
                    ),*
                ]);
...
impl<T: IntoScript> IntoScriptMulti for T {
    fn into_script_multi(self, world: WorldGuard) -> Result<VecDeque<ScriptValue>, InteropError> {
        self.into_script(world).map(|v| VecDeque::from_iter([v]))
    }
}

variadics_please::all_tuples!(impl_into_script_multi_tuple, 1, 14, T);
