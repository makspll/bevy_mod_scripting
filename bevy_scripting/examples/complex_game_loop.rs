...
fn fire_random_event(w: &mut PriorityEventWriter<LuaEvent>, events : &[ScriptEventData]){
    ...
    let (event,prio) = events.choose(&mut rng)
                                            .map(|v| (LuaEvent{ hook_name: v.0.to_string(), args: vec![arg] },v.1))
                                            .unwrap();
    
    info!("\t - event: {},\t prio: {},\t id: {}", event.hook_name,prio, id);
    w.send(event,prio);
}
...
static ALL_EVENTS : [ScriptEventData;6] = [ON_PRE_PHYSICS_ONE,
                                        ON_PRE_PHYSICS_TWO,
                                        ON_POST_PHYSICS_ONE,
                                        ON_POST_PHYSICS_TWO,
                                        ON_POST_UPDATE_ONE,
...
fn main() -> std::io::Result<()> {
    ...
    app.add_plugins(DefaultPlugins)
        ...
        .add_system_to_stage(PHYSICS,do_physics.with_run_criteria(FixedTimestep::step(TIMESTEP_2_PER_SECOND)))

        // pre physics logic stage (twice a second)
        .add_stage_before(PHYSICS,PRE_PHYSICS, SystemStage::parallel())
        .add_system_to_stage(PRE_PHYSICS,do_some_shit_before_physics.with_run_criteria(FixedTimestep::step(TIMESTEP_2_PER_SECOND)))
        ...
        .add_stage_before(PHYSICS,PRE_PHYSICS_SCRIPTS, SystemStage::single_threaded())
        .add_script_handler_stage_with_criteria::<RLuaScriptHost<LuaAPIProvider>,_,_,_,0,10>(PRE_PHYSICS_SCRIPTS,FixedTimestep::step(TIMESTEP_2_PER_SECOND))

        // post_physics,    priority: [11,20] inclusive 
        // since the previous system will consume all events in the [0,10] range
        .add_stage_after(PHYSICS,POST_PHYSICS_SCRIPTS, SystemStage::single_threaded())
        .add_script_handler_stage_with_criteria::<RLuaScriptHost<LuaAPIProvider>,_,_,_,11,20>(POST_PHYSICS_SCRIPTS,FixedTimestep::step(TIMESTEP_2_PER_SECOND))
        ...
        .add_stage_after(CoreStage::Update,POST_UPDATE_SCRIPTS, SystemStage::single_threaded())
